...
class OrderFormFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    protected function createFormAdultFullCourse(ObjectManager $manager): void
    {
        ...
        $form->getFields()->remove(4);
        $form->getFields()->remove(5);

        $field = (new OrderFormField($form))
            ->setType(OrderFormField::TYPE_ALLOWANCE_CHOICE)->setRequired(false)
            ->setQuestion('Êtes-vous concerné par le tarif réduit (étudiant, demandeur d\'emploi etc. ?')
        ;
