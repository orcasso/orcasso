...
final class PaymentControllerTest extends AbstractWebTestCase
{
    ...
    {
        $this->assertRedirectToLogin(Request::METHOD_GET, $this->getUrl('payment_list'));
        $this->assertRedirectToLogin(Request::METHOD_GET, $this->getUrl('payment_list_ajax'));
    ...
    public function testList()
    {
        ...
        $this->client->request(Request::METHOD_GET, $this->getUrl('payment_list'));
        $this->assertResponseIsSuccessful();
        $this->assertHasHtmlTitle('payment.list.title', [], null, 'h4');
    }

    public function testListAjax()
    {
        $this->authenticateUser();
        $this->client->request(Request::METHOD_GET, $this->getUrl('payment_list_ajax'));
        $this->assertResponseIsSuccessful();
    ...
    public function testDelete()
    {
        ...
        $this->assertEquals($paymentCount - 1, $this->getDoctrine()->getRepository(Payment::class)->count());
        $this->assertTrue($this->client->getResponse()->isRedirect($this->getUrl('payment_list')));
    ...
    {
        return $this->getUrl('payment_edit', ['payment' => $payment->getId()]);
    }

    protected function getEditHeaderUrl(Payment $payment): string
    {
        return $this->getUrl('payment_edit_header', ['payment' => $payment->getId()]);
    }

    protected function getCreateUrl(): string
    {
        return $this->getUrl('payment_create');
    }

    protected function getDeleteUrl(Payment $payment): string
    {
        return $this->getUrl('payment_delete', ['payment' => $payment->getId()]);
    }
}
