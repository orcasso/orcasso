...
final class OrderLineControllerTest extends AbstractWebTestCase
{
    ...
    public function testEdit()
    {
        ...
        $this->assertTrue($this->client->getResponse()->isRedirect($this->getUrl('order_edit', ['order' => $line->getOrder()->getId()])));
    ...
    public function testCreate()
    {
        ...
        foreach ([OrderLine::TYPE_SIMPLE, OrderLine::TYPE_ALLOWANCE, OrderLine::TYPE_ACTIVITY_SUBSCRIPTION] as $type) {
            ...
            $this->assertTrue($this->client->getResponse()->isRedirect($this->getUrl('order_edit', ['order' => $order->getId()])));
    ...
    {
        return $this->getUrl('order_line_edit', ['order' => $line->getOrder()->getId(), 'line' => $line->getId()]);
    }

    protected function getCreateUrl(Order $order, string $type): string
    {
        return $this->getUrl('order_line_create', ['order' => $order->getId(), 'type' => $type]);
    }

    protected function getDeleteUrl(OrderLine $line): string
    {
        return $this->getUrl('order_line_delete', ['order' => $line->getOrder()->getId(), 'line' => $line->getId()]);
    }
}
