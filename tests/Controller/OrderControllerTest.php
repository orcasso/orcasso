...
final class OrderControllerTest extends AbstractWebTestCase
{
    ...
    {
        $this->assertRedirectToLogin(Request::METHOD_GET, $this->getUrl('order_list'));
        $this->assertRedirectToLogin(Request::METHOD_GET, $this->getUrl('order_list_ajax'));
    ...
    public function testList()
    {
        ...
        $this->client->request(Request::METHOD_GET, $this->getUrl('order_list'));
        $this->assertResponseIsSuccessful();
        $this->assertHasHtmlTitle('order.list.title', [], null, 'h4');
    }

    public function testListAjax()
    {
        $this->authenticateUser();
        $this->client->request(Request::METHOD_GET, $this->getUrl('order_list_ajax'));
        $this->assertResponseIsSuccessful();
    ...
    public function testDelete()
    {
        ...
        $this->assertEquals($orderCount - 1, $this->getDoctrine()->getRepository(Order::class)->count());
        $this->assertTrue($this->client->getResponse()->isRedirect($this->getUrl('order_list')));
    ...
    {
        return $this->getUrl('order_edit', ['order' => $order->getId()]);
    }

    protected function getEditHeaderUrl(Order $order): string
    {
        return $this->getUrl('order_edit_header', ['order' => $order->getId()]);
    }
    ...
    {
        return $this->getUrl('order_delete', ['order' => $order->getId()]);
    }
}
